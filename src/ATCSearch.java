...
public class ATCSearch {
	...
	public static  ArrayList<String> ATC_name = new ArrayList<String>();

	
	public ATCSearch(){
		ATC_name = new ArrayList<String>();
	}
	
	 static String indexPath = "/media/jeyasoth1u/86562BD7562BC731/GMD/Index/ATC_Index1";
			...
			public static void doPagingSearch(BufferedReader in, IndexSearcher searcher, Query query, int hitsPerPage, boolean raw, boolean interactive) throws IOException {
				...
				while (true) {
					  ...
					  if (path != null) {
					  	...
					  	if (title != null) {
					  		if (!ATC_name.contains(doc.get("name"))){
					  		ATC_name.add(doc.get("name"));
					  		System.out.println((i+1) + ". "+"Name :" + doc.get("name"));
					  		}
					  	}
					  } else {
